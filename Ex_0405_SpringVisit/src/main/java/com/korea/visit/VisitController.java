...
public class VisitController {
	...
	@Autowired // 자동주입: spring으로부터 자동생성 가능한 객체를 new없이 알아서 생성해 준다// ServletContext app;도 가능
	...
	public String insert(VisitVO vo) {
		...
		if (!photo.isEmpty()) {
			...
			if(!saveFile.exists()) {
				saveFile.mkdirs(); //절대경로에 upload폴더를 생성한다.
			}else {
	...
	public String delete(int idx) {
		...
		// @ResponseBody의 속성을 통해 반환된 result 값이 자동으로 콜백 메서드로 전달
		return result;
	}
	...
	@ResponseBody // ResponseBody가 있어야 return 값을 콜백 메서드로 돌려줄 수 있다.
